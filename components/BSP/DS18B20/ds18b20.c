...
static const char *TAG = "DS18B20"; // 日志打印标识

/*************************初始化操作*************************/
unsigned char init_DS18(void)
{
    unsigned int init_state = 0;
    static int debug_ds18 = -10;
...
#define DS18B20_RETRY_NUM 3
#define DS18B20_INVALID_T 999
    /*************************温度转换过程*************************/
    for (i = 0; i < DS18B20_RETRY_NUM; i++)
    {
        if (init_DS18())
            break;
    }
    if (i >= DS18B20_RETRY_NUM)
    {
        printf("ds18b20 init fail!\n");
        return DS18B20_INVALID_T;
    }
    ...
    /*************************读取温度过程*************************/
    for (int i = 0; i < DS18B20_RETRY_NUM; i++)
    {
        if (init_DS18())
            break;
    }
    ...
    // printf("Th=%x,TL=%x,T=%f\n", TEMP_HIGH,TEMP_LOW,TEMP);
    // ESP_LOGE(TAG, "temp:%f\n", TEMP);
    return TEMP;
}